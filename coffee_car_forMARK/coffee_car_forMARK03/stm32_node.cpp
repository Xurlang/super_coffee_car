...
void stepMotorCallback(const std_msgs::Float64::ConstPtr &msg)
{
    if (!ser.isOpen())
    {
        ROS_ERROR("Serial port not open");
    ...
    int rpm = static_cast<int>(msg->data);
    if (rpm > 32767)
    {
        rpm = 32767;
    }
    ...
    int16_t step_rpm = static_cast<int16_t>(rpm);

    uint8_t send_buf[4] = {0};
    send_buf[0] = 0xBB;
    send_buf[1] = static_cast<uint8_t>((step_rpm >> 8) & 0xFF);
    send_buf[2] = static_cast<uint8_t>(step_rpm & 0xFF);
    send_buf[3] = 0x55;

    ser.write(send_buf, 4);

    ROS_INFO("Sent step_motor_rpm: %d", step_rpm);
}
...
int main(int argc, char **argv)
{
    ...
    ros::Subscriber step_motor_sub = nh.subscribe<std_msgs::Float64>("/step_motor_rpm", 100, stepMotorCallback);
