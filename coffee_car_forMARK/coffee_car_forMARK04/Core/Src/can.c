...
// 2026/3/19 新增：步进电机速度控制函数
// 通过 CANopen 对象字典 0x60FF 写入目标速度（rpm），正数正转，负数反转，0停止
void step_motor_set_velo(uint32_t ID, int16_t rpm)
{
    int32_t rpm_32 = (int32_t)rpm;  // 2026/3/19 新增：符号扩展到32位，避免负转速被错误打包
    ...
    // 2026/3/19 注释：原来只写低16位，负数会被当成大正数
    // CAN_Tx_Data[4] = (uint8_t)(rpm & 0xFF);
    // CAN_Tx_Data[5] = (uint8_t)((rpm >> 8) & 0xFF);
    // CAN_Tx_Data[6] = 0x00;
    // CAN_Tx_Data[7] = 0x00;

    // 2026/3/19 新增：按0x60FF的32位有符号速度完整发送
    CAN_Tx_Data[4] = (uint8_t)(rpm_32 & 0xFF);
    CAN_Tx_Data[5] = (uint8_t)((rpm_32 >> 8) & 0xFF);
    CAN_Tx_Data[6] = (uint8_t)((rpm_32 >> 16) & 0xFF);
    CAN_Tx_Data[7] = (uint8_t)((rpm_32 >> 24) & 0xFF);
