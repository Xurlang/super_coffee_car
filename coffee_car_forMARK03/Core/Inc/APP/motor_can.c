	...
	do{
		switch(jerk_process4)
			...
			case 1:
				{
					read_motor_state_5();
					if( 0x40 != ((ID2_Rx_Data_5 | ID3_Rx_Data_5) & 0x40)){		//2�ţ�3�ŵ�����ھ�ֹ״̬
							//�ջ�
							if(dire == 2)
							{
								read_motor_state_4();
...
uint8_t triangle_control(uint32_t ID, uint8_t dire)//���ǲ�
{
	do{
		...
		case 1:
				{
					read_motor_state_5();
					if( 0x40 != (ID1_Rx_Data_5 & 0x40)){		//1�ŵ����ֹ״̬
							//���ǲ�����(�����Ƴ�)
							if(dire == 1)
							{
								read_motor_state_4();
...
uint8_t big_bin_control(uint32_t ID, uint8_t dire)//���
{
	...
	case 1:
		{
			read_motor_state_5();
			if( 0x40 != (ID1_Rx_Data_5 & 0x40)){		//1��,2�ŵ�����ھ�ֹ״̬
					//������У������ջأ�
					if(dire == 1)
					{
						read_motor_state_4();
...
void test_location_3(uint32_t ID, uint8_t dire)//���
{
	...
	case 1:
		{
			read_motor_state_5();
			if( 0x40 != ((ID1_Rx_Data_5 | ID2_Rx_Data_5) & 0x40)){		//1��,2�ŵ�����ھ�ֹ״̬
					//������У������ջأ�
					if(dire == 1)
					{
						read_motor_state_4();
