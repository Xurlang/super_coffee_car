    ...
    : as_(nh_, name, boost::bind(&AbbRobot::executeCB, this, _1), false), joint_count(6)
{
    ...
    // 2026/2/2
    servo_sub_ = nh_.subscribe("cmd_servo_vel", 1, &AbbRobot::servoCallback, this);
    // 初始化变量
    servo_cmd_vel_.resize(6, 0.0);
    is_servo_active_ = false;
...
void AbbRobot::jointStateUpdate()
{
    ...
    if (pub_flag == 1)
    {
        // ---------------- [新增 Servo 逻辑开始] ----------------
            if (is_servo_active_) 
            {
                double time_diff = (ros::Time::now() - last_servo_msg_time_).toSec();
                if (time_diff < 0.2)
                {
                    trajectory_msgs::JointTrajectoryPoint servo_pt; 
                    servo_pt.positions.resize(6);
                    servo_pt.velocities.resize(6);
                    servo_pt.accelerations.resize(6);

                    double dt = 0.2; 

                    for(int i=0; i<6; i++) {
                        double raw_vel = servo_cmd_vel_[i];

                        if (std::abs(raw_vel) < 0.001) {
                            raw_vel = 0.0;
                        }

                        servo_pt.velocities[i] = raw_vel;
                        servo_pt.positions[i] = msg.position[i] + (raw_vel * dt);
                        servo_pt.accelerations[i] = 0.0;
                    }
...
// --- [新增 Servo 回调函数] ---2026/2/2
void AbbRobot::servoCallback(const std_msgs::Float64MultiArray::ConstPtr& msg)
{
    int incoming_size = msg->data.size();
    if (incoming_size < 5) {
        std::cout << "ERROR: Too few joints! Got " << incoming_size << std::endl;
        return; 
    }
    last_servo_msg_time_ = ros::Time::now();
    servo_cmd_vel_.resize(6, 0.0); 
    for(int i=0; i < incoming_size && i < 6; i++) {
        servo_cmd_vel_[i] = msg->data[i];
    }
    
    is_servo_active_ = true;
}