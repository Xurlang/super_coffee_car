...
void recordPhase() {
    ...
    std::thread input_thread([&stop_recording]() {
        // 阻塞等待用户按下回车
    ...
    // 主循环：只要没按下回车，且ROS正常，就一直跑
    while (ros::ok() && g_running && !stop_recording) {
        ...
        // 打印状态 (每秒一次)
        if ((ros::Time::now() - last_print_time).toSec() >= 1.0) {
            ROS_INFO("Recording... %.1f s | Frames: %lu", 
                     frame.timestamp, g_motion_buffer.size());
            last_print_time = ros::Time::now();
        }
    ...
    if (input_thread.joinable()) {
        // 如果是因为checkSafety跳出的循环，线程可能还在阻塞等待回车
        // 这种情况下我们不detach，而是提示用户按回车继续，保持逻辑简单
        if (!stop_recording) {
             ROS_WARN("Recording aborted early. Please press Enter to continue flow...");
        }
        input_thread.join();
    }

    ROS_INFO("Recording stopped. Total frames: %lu", g_motion_buffer.size());
}
...
int main(int argc, char** argv) {
    ...
    // 阶段1：示教记录（如果未跳过）
    if (!skip_record) {
        ROS_INFO("\n[Phase 1] Ready to Record.");
        ROS_INFO(">>> Press ENTER to START recording (Gravity Compensation ON) <<<");
        // 清空缓冲区并等待回车
        std::cin.ignore(std::numeric_limits<std::streamsize>::max(), '\n'); 
        std::cin.get();
        
        // 进入录制函数（函数内部会再次启动线程等待第二次回车来结束）
        recordPhase();
    ...
    // 阶段3：轨迹回放
    ROS_INFO("\n[Phase 3] Ready to Replay.");
    ROS_INFO(">>> Press ENTER to START replay <<<");
    // 这里不需要 ignore 了，因为 recordPhase 里的线程已经处理掉了上一个回车
    // 但为了保险起见，可以用 peek 判断一下，或者直接由用户控制
    // 简单起见，直接等待：
    std::cin.get(); 
