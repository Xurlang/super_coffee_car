...
private:
    ...
    // --- [新增 Servo 支持变量] ---2026/2/2
    void servoCallback(const std_msgs::Float64MultiArray::ConstPtr& msg);
    ros::Subscriber servo_sub_;           // 订阅者
    std::vector<double> servo_cmd_vel_;   // 存速度
    ros::Time last_servo_msg_time_;       // 存时间
    bool is_servo_active_;                // 存状态
